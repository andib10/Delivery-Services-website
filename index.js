...
// instantaBD.select({campuri:["nume","pret"],tabel:"colete", conditiiAnd:["pret>10", "pret<20"]}, function(err, rez){
...

{
    ...
    (async function(){
    let u = await Utilizator.getUtilizDupaUsernameAsync("prof59486");
...
app.post("/produse_cos",function(req, res){
    ...
    if(req.body.ids_prod.length!=0){
        ...
        AccesBD.getInstanta().select({tabel:"colete", campuri:"nume,descriere,pret,zile_livrare,imagine".split(","),conditiiAnd:[`id in (${req.body.ids_prod})`]},
        function(err, rez){
            if(err)
                res.send([]);
            else
...
client.query("select id from colete", function(err, rez){
    ...
    formular.parse(req, function(err, campuriText, campuriFisier){
        Utilizator.getUtilizDupaUsername (campuriText.username,{
            ...
            if(u.parola == parolaCriptata && u.confirmat_mail){
                ...
                obparam.res.render("pagini/index", {eroareLogin:"Date logare incorecte sau nu a fost confirmat mailul!", imagini:obGlobal.imagini})
