...
class Utilizator{
    ...
    static emailServer= "testnodetesting@gmail.com"
    #eroare;
    ...
    salvareUtilizator(){
        ...
        AccesBD.getInstanta(Utilizator.tipConexiune).insert({tabel:Utilizator.tabel, campuri:["username",
        "nume","prenume","parola","email","culoare_chat","cod","poza"], valori:[`'${this.username}'`,
        `'${this.nume}'`,`'${this.prenume}'`,`'${parolaCriptata}'`,`'${this.email}'`,
        `'${this.culoare_chat}'`,`'${token}'`,`'${this.poza}'`]}, function(err, rez){
            if(err)
                console.log(err);
    ...
    static getUtilizDupaUsername (username, obparam, proceseazaUtiliz){
        if (!username) 
            return null;
        let eroare = null;
        AccesBD.getInstanta(Utilizator.tipConexiune).select({tabel:"utilizatori", campuri:['*'],
        conditiiAnd:[`username=$1`]}, function (err, rezSelect){
            if(err ){
